...
class DDPG:
    ...
    Parameters:
        ...
        device (torch.device, optional): Device to run the computations on (CPU or GPU).
            Default is torch.device("cuda" if torch.cuda.is_available() else "cpu").
        ...
        tau (float, optional): Soft update parameter for the actor target network. Default is 0.01.
        ...
        critic_clamping_max_limit (float, optional): Maximum value for clamping critic outputs. Default is 100.
    ...
    """
    def __init__(self, state_dim, action_dim, N_t, K, P_max, #action_bound
        ...
        self.K = K
        self.P_max = P_max
        self.total_it = 0  # Counter for delayed actor updates
        self.device = device
        ...
        self.scaler = torch.GradScaler(self.device) # Initialize the GradScaler
        ...
        self.critic = critic_model(state_dim, action_dim,
                                   critic_linear_layers = critic_linear_layers).to(self.device)
        self.target_critic = critic_model(state_dim, action_dim,
                                          critic_linear_layers = critic_linear_layers).to(self.device)
        ...
        self.critic_optimizer = OPTIMIZERS[optimizer_name](self.critic.parameters(), lr=critic_lr) # we want to minimize the critic loss
    ...
    def _sample_from_buffer(self, batch_size):
        ...
        Returns a unified tuple: (states, actions, rewards, next_states, weights, indices)
        """
        if self.use_per:
            return self.replay_buffer.sample(batch_size)
        else:
            state, actions, rewards, next_state = self.replay_buffer.sample(batch_size)
            dummy_weights = torch.ones(batch_size, device=self.device)
            dummy_indices = np.arange(batch_size)
            return state, actions, rewards, next_state, dummy_weights, dummy_indices
    ...
    def training(self, batch_size):
        ...
        Returns:
            actor_loss (tensor): Loss of the actor network.
            critic_loss (tensor): Loss of the critic network.
            target_q_values (tensor): Target Q-values.
            y (tensor): Target values for the critic.
            rewards (tensor): Rewards from the sampled batch.
            q_values (tensor): Q-values predicted by the critic.
        """
        # * passing the actor in training mode and moving it to the GPU for faster training
        self.actor.train()
        self.total_it += 1
        ...
        if self.gpu_used:
            state, actions, rewards, next_state = (t.to(self.device, non_blocking=True) for t in (state, actions, rewards, next_state))
            if self.use_per:
        ...
        if self.using_loss_scaling:
            if self.total_it % self.critic_frequency_update == 0:
                ...
                with amp.autocast(self.device_string):
                    ...
                    self.scaler.update()
                if self.use_per:
                    td_errors = self._calculate_td_errors(state, actions, rewards, next_state, q_values)
    ...
    def update_target_networks(self, update_target_actor = True, update_target_critic = True):
        ...
        Parameters:
            update_target_actor (bool): Whether to update the target actor network.
            update_target_critic (bool): Whether to update the target critic network.
        """
        with torch.no_grad():
    ...
    def save_models(self, directory):
        ...
        Parameters:
            directory (str): Path to the directory where the models should be saved.
        """
        ...
        target_critic_path = os.path.join(directory, "target_critic.pth")
        # Save the models
...
class Custom_DDPG:
    """Custom DDPG implementation with additional critic networks for improved stability and performance, espcially to avoid divergence for the critic (while using TD learning). This is forced because the initial environment described in Peng & Al 2022 only uses positive rewards, learning to accumulating the errors for the critic.
    ...
    def __init__(self, state_dim, action_dim, N_t, K, P_max, #action_bound
                 ...
                 device = torch.device("cuda" if torch.cuda.is_available() else "cpu"),
                 optimizer = "adam",
                 actor_lr=0.0001, critic_lr=0.0005, gamma=0.99, tau=0.01,critic_tau=0.001,
                 buffer_size=10000, seed = 42,
        ...
        # ? Using two critics (and one target network for each for soft update)
        self.present_critic = critic_model(state_dim, action_dim,
                                           critic_linear_layers = critic_linear_layers).to(self.device)
        self.target_present_critic = critic_model(state_dim, action_dim,
                                                   critic_linear_layers = critic_linear_layers).to(self.device)
        self.target_present_critic.load_state_dict(self.present_critic.state_dict())

        self.future_critic = critic_model(state_dim, action_dim,
                                           critic_linear_layers = critic_linear_layers).to(self.device)
        self.target_future_critic = critic_model(state_dim, action_dim,
                                                  critic_linear_layers = critic_linear_layers).to(self.device)
        self.target_future_critic.load_state_dict(self.future_critic.state_dict())
        ...
        optimizer_name = optimizer.lower()
        self.actor_optimizer = OPTIMIZERS[optimizer_name](self.actor.parameters(), lr=actor_lr, maximize=True) # we want to maximize the state-value function (the actor)
        self.q_optimizer = OPTIMIZERS[optimizer_name](list(self.future_critic.parameters()) + list(self.present_critic.parameters()), lr=critic_lr) # we want to minimize the critic loss
    ...
    def _calculate_td_errors(self, future_q_values, y_future_reward):
        """
        Calculates TD errors for PER updates using the future critic's TD target.
        """
        with torch.no_grad():
            td_errors = torch.abs(future_q_values - y_future_reward)
        return td_errors.detach().cpu().numpy().flatten()
    ...
    def select_noised_action(self, state, noise_scale=0.01):
        ...
        # Generate noise using your seeded numpy RNG
        noise_np = self.network_numpy_rng.normal(loc=0.0, scale=noise_scale, size=raw_action.shape)
        noise = torch.tensor(noise_np, dtype=torch.float32, device=self.device)
    ...
    def training(self, batch_size):
        ...
        if self.using_loss_scaling:
            if self.total_it % self.critic_frequency_update == 0:
                ...
                with amp.autocast(self.device_string):
                    ...
                    if self.use_per:
                        present_reward_critic_loss = (weights * F.mse_loss(present_q_values, y_present_reward, reduction='none')).mean()
                        future_reward_critic_loss = (weights * F.mse_loss(future_q_values, y_future_reward, reduction='none')).mean()
                    else:
                    ...
                    combined_critics_loss = present_reward_critic_loss + future_reward_critic_loss
                    # * update the critics
                    # * The future critic is used as the actor loss and is supposed to represent the expected reward (cf TD error)
                    self.q_optimizer.zero_grad()
                    self.scaler.scale(combined_critics_loss).backward()
                    self.scaler.step(self.q_optimizer)
                    self.scaler.update()

                if self.use_per:
                    td_errors = self._calculate_td_errors(future_q_values, y_future_reward)
                    new_priorities = td_errors + self.per_epsilon
                    self.replay_buffer.update_priorities(indices, new_priorities)
                update_target_critic = True
        ...
        #* not using mixed precision
            ...
            if self.total_it % self.critic_frequency_update == 0:
                ...
                self.q_optimizer.zero_grad()
                combined_critics_loss.backward()
        ...
        with torch.no_grad():
            if update_target_actor:
                for target_param, param in zip(self.target_actor.parameters(), self.actor.parameters()):
                    target_param.data.copy_(self.tau * param.data + (1 - self.tau) * target_param.data)
            if update_target_critic:
                for target_param, param in zip(self.target_present_critic.parameters(), self.present_critic.parameters()):
                    target_param.data.copy_(self.critic_tau * param.data + (1 - self.critic_tau) * target_param.data)
                for target_param, param in zip(self.target_future_critic.parameters(), self.future_critic.parameters()):
                    target_param.data.copy_(self.critic_tau * param.data + (1 - self.critic_tau) * target_param.data)
    ...
    def save_models(self, directory):
        ...
        # Define the file paths
        actor_path = os.path.join(directory, "actor.pth")
        present_critic_path = os.path.join(directory, "present_critic.pth")
        future_critic_path = os.path.join(directory, "future_critic.pth")
        target_actor_path = os.path.join(directory, "target_actor.pth")
        target_present_critic = os.path.join(directory, "target_present_critic.pth")
        target_future_critic = os.path.join(directory, "target_future_critic.pth")
        # Save the models
        torch.save(self.actor.state_dict(), actor_path)
        torch.save(self.target_actor.state_dict(), target_actor_path)
        torch.save(self.present_critic.state_dict(), present_critic_path)
        torch.save(self.target_present_critic.state_dict(), target_present_critic)
        torch.save(self.future_critic.state_dict(), future_critic_path)
        torch.save(self.target_future_critic.state_dict(), target_future_critic)