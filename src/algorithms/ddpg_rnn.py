...
class DDPG_RNN:
    ...
    def _sample_from_buffer(self, batch_size):
        ...
        if self.sequence_length > 1:
            # Sequence-aware buffer sampling
            if self.use_per:
        ...
        if self.using_loss_scaling:
            if self.total_it % self.critic_frequency_update == 0:
                ...
                with amp.autocast(self.device_string):
                    q_values, _ = self.critic(state, actions)
                    if self.use_per:
    ...
    def store_transition(self, state, action, reward, next_state, done=False, batch_size=None):
        """Stores a transition in the replay buffer."""
        if self.sequence_length > 1:
            # For sequence-aware buffers, we need to pass the done flag
            self.replay_buffer.add(state, action, reward, next_state, done=done, batch_size=batch_size)
        else:
...
class Custom_DDPG_RNN:
    ...
    def __init__(self, state_dim, action_dim, N_t, K, P_max,
        ...
        self.present_critic = RNNCriticNetwork(
            state_dim=state_dim, action_dim=action_dim,
            rnn_type=rnn_type, rnn_hidden_size=rnn_hidden_size, rnn_num_layers=rnn_num_layers,
            critic_linear_layers=critic_linear_layers, sequence_length=sequence_length
        ).to(self.device)
        
        self.target_present_critic = RNNCriticNetwork(
            state_dim=state_dim, action_dim=action_dim,
            rnn_type=rnn_type, rnn_hidden_size=rnn_hidden_size, rnn_num_layers=rnn_num_layers,
            critic_linear_layers=critic_linear_layers, sequence_length=sequence_length
        ).to(self.device)
        
        self.future_critic = RNNCriticNetwork(
            state_dim=state_dim, action_dim=action_dim,
            rnn_type=rnn_type, rnn_hidden_size=rnn_hidden_size, rnn_num_layers=rnn_num_layers,
            critic_linear_layers=critic_linear_layers, sequence_length=sequence_length
        ).to(self.device)
        
        self.target_future_critic = RNNCriticNetwork(
            state_dim=state_dim, action_dim=action_dim,
            rnn_type=rnn_type, rnn_hidden_size=rnn_hidden_size, rnn_num_layers=rnn_num_layers,
            critic_linear_layers=critic_linear_layers, sequence_length=sequence_length
        ).to(self.device)
    ...
    def training(self, batch_size):
        ...
        with torch.no_grad():
            ...
            y_future_reward = rewards + self.gamma * target_q_values
        ...
        if self.using_loss_scaling:
            if self.total_it % self.critic_frequency_update == 0:
                updated_critic = True
                with amp.autocast(self.device_string):
                    present_q_values, _ = self.present_critic(state, actions)
                    future_q_values, _ = self.future_critic(state, actions)
        ...
        if self.gpu_used:
            actor_loss = actor_loss.to('cpu')
            present_reward_critic_loss = present_reward_critic_loss.to('cpu')
    ...
    def save_models(self, directory):
        ...
        target_actor_path = os.path.join(directory, "target_actor.pth")
        target_present_critic_path = os.path.join(directory, "target_present_critic.pth")
        target_future_critic_path = os.path.join(directory, "target_future_critic.pth")
        
        # Save the models
        torch.save(self.actor.state_dict(), actor_path)
        torch.save(self.target_actor.state_dict(), target_actor_path)
        torch.save(self.present_critic.state_dict(), present_critic_path)
        torch.save(self.target_present_critic.state_dict(), target_present_critic_path)
        torch.save(self.future_critic.state_dict(), future_critic_path)
        torch.save(self.target_future_critic.state_dict(), target_future_critic_path)
    ...
    def load_models(self, directory):
        ...
        self.actor.load_state_dict(torch.load(actor_path, map_location=self.device))
        self.target_actor.load_state_dict(torch.load(target_actor_path, map_location=self.device))
        self.present_critic.load_state_dict(torch.load(present_critic_path, map_location=self.device))
        self.target_present_critic.load_state_dict(torch.load(target_present_critic_path, map_location=self.device))
        self.future_critic.load_state_dict(torch.load(future_critic_path, map_location=self.device))
        self.target_future_critic.load_state_dict(torch.load(target_future_critic_path, map_location=self.device))
