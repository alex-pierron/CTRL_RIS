...
This module provides:
 ...
 - multiprocess_trainer: the main loop coordinating vectorized environments
...
def ofp_multiprocess_trainer(training_envs, network, training_config, log_dir, writer,
                         ...
                         use_rendering = False):
    """
    Executes the training loop for the network with improved logging in a multiprocess setup.
    ...
    # NOTE: Progress bar for initial replay buffer warmup
    filling_buffer_progress_bar = tqdm(total=buffer_number_of_required_episode, desc="FILLING REPLAY BUFFER", position=0, ascii="->#", leave=True) if not batch_instead_of_buff else None
    ...
    for episode in tqdm(range(num_episode), desc="TRAINING", position=1, ascii="->#"):
        ...
        for num_step in range(max_num_step_per_episode):
            ...
            if action_noise_activated:
                # NOTE: Per-env exploration noise; actions and next states computed vectorially
                ...
                for i in range(n_rollout_envs): #? To optimize and maybe vmap
                ...
                # NOTE: Deterministic actions across vectorized envs
            ...
            # NOTE: Store transitions for all envs in a single batch operation
